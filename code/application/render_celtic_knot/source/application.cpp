    ...
    : DscWindows::IWindowApplication(in_hwnd, in_fullScreen, in_defaultWidth, in_defaultHeight)
{
    ...
    {
        std::vector<uint8> vertex_shader_data;
        if (false == _file_system->LoadFile(vertex_shader_data, DscCommon::FileSystem::JoinPath("shader", "data_vs.cso")))
        {
            DSC_LOG_WARNING(LOG_TOPIC_APPLICATION, "failed to load vertex shader\n");
        }
        std::vector<uint8> pixel_shader_data;
        if (false == _file_system->LoadFile(pixel_shader_data, DscCommon::FileSystem::JoinPath("shader", "data_ps.cso")))
        {
            DSC_LOG_WARNING(LOG_TOPIC_APPLICATION, "failed to load pixel shader\n");
        }
        std::vector < DXGI_FORMAT > render_target_format;
        ...
        _data_shader_constant_buffer = _data_shader->MakeShaderConstantBuffer(_draw_system.get());
    }
    ...
    //_data_render_target
    {
        ...
        _data_render_target = std::make_shared<DscRenderResource::RenderTargetTexture>(
            _draw_system.get(),
            target_format_data_array,
            DscRender::RenderTargetDepthData(),
            DscCommon::VectorInt2(16, 16)
            );
    }
...
Application::~Application()
{
    ...
    _data_shader.reset();
    _data_shader_constant_buffer.reset();
    _data_render_target.reset();
