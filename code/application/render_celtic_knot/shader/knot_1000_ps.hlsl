...
float CalculateArc(float in_value, float2 in_pos, float in_radius, float in_thickness, float2 in_clip_dir)
{
    ...
    float c = dot(in_clip_dir, pos);
    float value_c = max(0.0, (c * 512.0) + 1.0);

    float result = max(in_value, min(value_d, value_c));
    return result;
}
