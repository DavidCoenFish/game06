...
namespace TestUtil
{
	...
	template<typename TYPE>
	const bool EqualVector(
		const bool in_ok, 
		const std::vector<TYPE>& in_valueLhs, 
		const std::vector<TYPE>& in_valueRhs, 
		const std::string& in_fileName, 
		const int in_lineNumber
		)
	{
		bool match = false;
	...
	const bool AlmostEqual(
		const bool in_ok, 
		const float in_valueLhs, 
		const float in_valueRhs, 
		const std::string& in_fileName, 
		const int in_lineNumber,
		const float in_epsilon = std::numeric_limits<float>::epsilon()
		);
...
#define TEST_UTIL_ALMOST_EQUAL_EPSILON(OK, LHS, RHS, EPSILON) TestUtil::AlmostEqual(OK, LHS, RHS, __FILE__, __LINE__, EPSILON);
