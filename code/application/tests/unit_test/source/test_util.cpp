...
const bool TestUtil::AlmostEqual(const bool in_ok, const float in_valueLhs, const float in_valueRhs, const std::string& in_fileName, const int in_lineNumber, const float in_epsilon)
{
	if (in_epsilon < std::abs(in_valueLhs - in_valueRhs))
	{
