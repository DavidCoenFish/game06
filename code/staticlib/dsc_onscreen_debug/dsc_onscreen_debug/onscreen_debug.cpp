...
DscOnscreenDebug::OnscreenDebug::OnscreenDebug(
...
{
    _font = in_text_manager.LoadFont(in_file_system, DscCommon::FileSystem::JoinPath("data", "font", "code2000.ttf"));
    ...
    _text_run = std::make_unique<DscText::Text>(
        ...
        false,
        0,
        DscText::THorizontalAlignment::TLeft,
    ...
    _screen_quad = std::make_unique<DscUi::ScreenQuad>(
        ...
        DscUi::VectorUiCoord2(DscUi::UiCoord(0, 0.0f), DscUi::UiCoord(0, 1.0f)),
        DscUi::VectorUiCoord2(DscUi::UiCoord(0, 0.0f), DscUi::UiCoord(0, 1.0f)),
        container_size
        );
    ...
    {
        DSC_LOG_WARNING(LOG_TOPIC_DSC_ONSCREEN_DEBUG, "failed to load triangle vertex shader\n");
    }
    ...
    {
        DSC_LOG_WARNING(LOG_TOPIC_DSC_ONSCREEN_DEBUG, "failed to triangle load pixel shader\n");
    }
...
void DscOnscreenDebug::OnscreenDebug::SetText(
    const std::string& in_text
)
{
    if (in_text != _debug_text)
    {
        _debug_text_dirty = true;
        _debug_text = in_text;
    }
}


void DscOnscreenDebug::OnscreenDebug::Update(
    DscRender::DrawSystem& in_draw_system,
...
{
    // update text run
    if (_debug_text_dirty)
    {
        _debug_text_dirty = false;
        ...
        _text_run->SetTextRunArray(std::move(text_run_array));

        DscCommon::VectorInt2 text_size = _text_run->GetTextBounds();
        _text_run->SetTextContainerSize(text_size);

        _screen_quad->SetQuadSize(
            DscUi::VectorUiCoord2(DscUi::UiCoord(text_size.GetX(), 0.0f), DscUi::UiCoord(text_size.GetY(), 0.0f)),
            DscUi::VectorUiCoord2(DscUi::UiCoord(0, 0.0f), DscUi::UiCoord(0, 1.0f)),
            DscUi::VectorUiCoord2(DscUi::UiCoord(0, 0.0f), DscUi::UiCoord(0, 1.0f))
            );

        in_frame.UpdateRenderTargetSize(_render_target_texture, text_size);
    }
...
void DscOnscreenDebug::OnscreenDebug::OnDeviceLost()
{
    //nop
}

void DscOnscreenDebug::OnscreenDebug::OnDeviceRestored(
...
void DscOnscreenDebug::OnscreenDebug::OnResize(
